...
/**
    ...
    example I have not formally covered maps yet. But I am sure
...
public final class Kevin {
    ...
    private static void solveBacon() {
        ...
        while(!q.isEmpty()&&!found) {
            Vertex<String> x = q.remove();
            if(graph.label(x) == null) {
                for (Edge<String> e: graph.outgoing(x)){
                    graph.label(e, true);
                    q.add(graph.to(e));
                }
            

                for(Edge<String> e: graph.incoming(x)) {
                    if(graph.label(e) ==true) {
                    graph.label(x, graph.from(e));
                    }
                }
       ...
       if(!found) {
           System.out.println("Error: Path to actor from Kevin Bacon does not exist");
           System.exit(1);
       }
       Vertex<String> y = actor;
       while(y != bacon) {

           System.out.println(y.get());
           if (graph.label(y) instanceof Vertex<?>) {
               y = (Vertex<String>) graph.label(y);
           }
       }
       System.exit(0);
