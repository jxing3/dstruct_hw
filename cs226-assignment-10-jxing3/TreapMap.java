...
implements OrderedMap<K, V> {
    ...
    private Node remove(Node n, K k) {
        ...
        int cmp = k.compareTo(n.key);
        if (cmp < 0) {
            n.left = this.checkPriority(this.remove(n.left, k));
        } else if (cmp > 0) {
            n.right = this.checkPriority(this.remove(n.right, k));
        } else {
    ...
    public static void main() {
        OrderedMap<Integer, String> list = new TreapMap<Integer, String>();
        list.insert(1, "a");
        list.insert(2, "b");
        list.insert(4, "c");
        list.insert(3, "d");
